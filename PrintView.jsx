...
/* ─────────────────────────────────────────────
   A4 Landscape: 297 × 210 mm  →  1122 × 794px (96dpi)
   Each <Page> is one sheet. @media print forces page-break.
───────────────────────────────────────────────── */
  ...
  <Page pageNum={8} total={TOTAL} bg={BG2}>
    ...
    <div style={{ background:BG3, borderRadius:20, border:"1px solid rgba(255,255,255,.06)", marginBottom:18, overflow:"hidden" }}>
      <table style={{ width:"100%", borderCollapse:"collapse" }}>
        ...
        <tbody>
          <Row label="월 평균 예약 건수" v1="65건" v2="140건" v3="230건" dim />
          <Row label="평균 거래금액" v1="60만원" v2="60만원" v3="60만원" dim />
          <Row label="연간 총 거래액" v1="4.68억원" v2="10.08억원" v3="16.56억원" dim />
          <Row label="총 플랫폼 수익" v1="1.54억원" v2="3.33억원" v3="5.47억원" />
          <Row label="총 운영 비용" v1="2.44억원" v2="2.84억원" v3="4.0억원" dim />
          <Row label="영업이익" v1="▼ 0.90억원" v2="▲ 0.49억원" v3="▲ 1.47억원" profit />
        </tbody>
      </table>
    </div>
...
const Slide9 = () => (
  <Page pageNum={9} total={TOTAL}>
    ...
    {/* NPS */}
    <div style={{ padding:"14px 18px", background:BG3, borderRadius:16, border:"1px solid rgba(255,255,255,.06)" }}>
      <div style={{ fontSize:11, fontWeight:700, color:TEXT, marginBottom:10 }}>NPS 목표</div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
        {[["2026","NPS 40+","초기 목표"],["2027","NPS 60+","성장 목표"],["2028","NPS 70+","업계 최고"]].map(([y,n,d])=>(
          <div key={y} style={{ padding:"10px 14px", borderRadius:12, background:`${LIME_DIM}`, border:`1px solid ${LIME_BDR}`, textAlign:"center" }}>
            <div style={{ fontSize:10, color:MUTED }}>{y}년</div>
            <div style={{ fontSize:20, fontWeight:900, color:LIME }}>{n}</div>
            <div style={{ fontSize:9, color:MUTED }}>{d}</div>
          </div>
        ))}
      </div>
    </div>
...
const Slide11 = () => (
  <Page pageNum={11} total={TOTAL}>
    <Label n="08" text="Team" />
    <H1 style={{ marginBottom:8 }}>필요 팀 구성</H1>
    <p style={{ fontSize:11, color:MUTED, marginBottom:24 }}>창업 초기 핵심 6인 체제로 운영하며, 개발은 외주 에이전시와 협업합니다.</p>
    ...
    <div style={{ fontSize:12, fontWeight:700, color:TEXT, marginBottom:12 }}>타겟 고객 페르소나</div>
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12 }}>
      {[["💔","슬픔의 동반자","30-45세 · 중산층","정보 부재로 인한 판단 능력 상실","투명 가격 비교 + 실시간 예약"],
        ["✨","명예로운 이별 추구자","25-35세 · 고학력","품질 불균일로 인한 신뢰 부족","100항목 인증 파트너 + 실시간 알림"],
        ["📱","실용주의자","35-55세 · 비용 효율 중시","가격 불투명 + 전화/방문 필요","3분 앱 예약 + 비교 플랫폼"]].map(([ic,nm,ag,pain,sol])=>(
        <div key={nm} style={{ padding:"14px 14px", borderRadius:16, background:"rgba(255,255,255,.04)", border:"1px solid rgba(255,255,255,.07)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
            <div style={{ fontSize:22 }}>{ic}</div>
            <div>
              <div style={{ fontSize:11, fontWeight:700, color:TEXT }}>{nm}</div>
              <div style={{ fontSize:9, color:MUTED }}>{ag}</div>
            </div>
          </div>
          <div style={{ fontSize:9, color:"#ff7070", fontWeight:700, marginBottom:2 }}>Pain</div>
          <div style={{ fontSize:10, color:MUTED, lineHeight:1.4, marginBottom:6 }}>{pain}</div>
          <div style={{ fontSize:9, color:LIME, fontWeight:700, marginBottom:2 }}>Solution</div>
          <div style={{ fontSize:10, color:MUTED, lineHeight:1.4 }}>{sol}</div>
        </div>
      ))}
    </div>
  </Page>
...
const Slide12 = () => (
  <Page pageNum={12} total={TOTAL} bg={BG2}>
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:680, textAlign:"center", position:"relative" }}>
      ...
      <div style={{ fontSize:10, letterSpacing:"0.2em", textTransform:"uppercase", color:LIME, fontWeight:700, marginBottom:20 }}>09 / Business Plan</div>
      ...
      <div style={{ display:"flex", gap:16 }}>
        <div style={{ padding:"14px 32px", background:LIME, color:"#0D0D14", fontWeight:700, borderRadius:14, fontSize:13 }}>사업계획 미팅 신청</div>
        <div style={{ padding:"14px 32px", border:"1px solid rgba(255,255,255,.15)", color:TEXT, fontWeight:700, borderRadius:14, fontSize:13 }}>사업계획서 다운로드</div>
      </div>
  ...
  return (
    <>
      ...
      <div id="print-toolbar" style={{
        ...
        background:"rgba(13,13,20,.95)", backdropFilter:"blur(20px)",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:16 }}>
          ...
          <div style={{ fontSize:11, color:MUTED }}>사업계획서 미리보기</div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <button onClick={()=>setCurrent(Math.max(0,current-1))} disabled={current===0}
            style={{ padding:"6px 14px", borderRadius:8, border:"1px solid rgba(255,255,255,.15)", background:"transparent", color:current===0?MUTED:TEXT, cursor:current===0?"not-allowed":"pointer", fontSize:12 }}>
            ← 이전
          </button>
          <span style={{ fontSize:11, color:MUTED, minWidth:60, textAlign:"center" }}>{current+1} / {TOTAL}</span>
          <button onClick={()=>setCurrent(Math.min(TOTAL-1,current+1))} disabled={current===TOTAL-1}
            style={{ padding:"6px 14px", borderRadius:8, border:"1px solid rgba(255,255,255,.15)", background:"transparent", color:current===TOTAL-1?MUTED:TEXT, cursor:current===TOTAL-1?"not-allowed":"pointer", fontSize:12 }}>
            다음 →
          </button>
          <button onClick={handlePrint}
            style={{ padding:"6px 18px", borderRadius:8, background:LIME, color:"#0D0D14", fontWeight:700, border:"none", cursor:"pointer", fontSize:12, marginLeft:8 }}>
            🖨 인쇄 / PDF 저장
          </button>
          {onClose && (
            <button onClick={onClose}
              style={{ padding:"6px 14px", borderRadius:8, border:"1px solid rgba(255,255,255,.15)", background:"transparent", color:MUTED, cursor:"pointer", fontSize:12 }}>
              닫기
            </button>
          )}
        </div>
      ...
      <div id="print-root" style={{ background:"#1a1a2e", minHeight:"100vh", paddingTop:60 }}>
        {/* ─── Screen: show one slide at a time ─── */}
        <div className="screen-view" style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"calc(100vh - 60px)", padding:"24px" }}>
          <div style={{ transform:"scale(0.85)", transformOrigin:"center top", boxShadow:"0 24px 80px rgba(0,0,0,.6)" }}>
            {slides[current]}
          </div>
        </div>
      ...
      <style>{`
        @media print {
          ...
          .print-all-slides { display: block !important; }
        }
      `}</style>
    </>
  );
